...
def get_user_points(img, max_width=1200, max_height=800):
    ...
    def mouse_callback(event, x, y, flags, param):
        nonlocal clicked_points, selected_point_idx

        if event == cv2.EVENT_LBUTTONDOWN:
            original_x = int(x / scale)
            original_y = int(y / scale)

            # Check if user clicked near an existing point
            for i, pt in enumerate(clicked_points):
                if inside((x, y), (pt[0] * scale, pt[1] * scale)):
                    selected_point_idx = i
                    return

            # If not editing, add new point
            if len(clicked_points) < 4:
                clicked_points.append((original_x, original_y))
                print(f"Point {len(clicked_points)}: ({original_x}, {original_y})")
        ...
        elif event == cv2.EVENT_LBUTTONUP and selected_point_idx != -1:
            selected_point_idx = -1
    ...
    while True:
        ...
        if key == 27:
            ...
            return None
        elif key == 13 and len(clicked_points) == 4:  # Enter key
            cv2.destroyWindow("Select 4 corners (Enter to preview, ESC to skip)")
...
if __name__ == "__main__":
    print("[START] Interactive Crop + Proper Ratio + Draggable Dots")
